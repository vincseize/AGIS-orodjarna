    ...
    def processAlgorithm(self, parameters, context, feedback):
       ...
       #Merge identical SU-s (same SU id, same raster source)        
        ...
        if len(error_raster) != 0: 
            feedback.reportError(self.tr('Fotoskice, ki jih ni bilo mogoče uporabiti: %s')  % str(error_raster))
        if len(error_unknown) != 0: 
            feedback.reportError(self.tr('Fotoskice, ki jih ni bilo mogoče obrezati: %s')  %str(error_unknown))
        return {}
